<template>
	<view>
		<!-- content -->
		<view class="cu-bar search bg-white nav fixed" :style="[{top:fixedTop+'px'}]" style="align-items: center;">
			<view class="search-form round">
				<text class="cuIcon-search"></text>
				<input class="search-input" @input="keywordInput" @confirm="searchClick" :adjust-position="false" type="text" placeholder="搜索图片、文章、视频"
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				console.log("getList：",this.page,",key:",this.key)
				this.showLoading = true;
				api.get(`/search/query/${this.key}/category/all/count/20/page/${this.page}`)
					.then(res => {
