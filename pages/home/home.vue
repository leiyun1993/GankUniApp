	...
	<view>
		<scroll-view scroll-y class="page">
			...
			<swiper class="card-swiper" :indicator-dots="false" :circular="true" :autoplay="true" interval="5000" duration="500"
			 @change="cardSwiper">
				<swiper-item v-for="(item,index) in meiziList" :key="index" :class="cardCur==index?'cur':''" @tap="onMeiZiItemClick" :data-url="item.url">
			...
			getTodayInfo() {
				api.get("/today")
					.then(res => {
						...
						// this.androidList = res.results["Android"];
						// this.appList = res.results["App"];
						// this.iosList = res.results["iOS"];
						// this.voideList = res.results["休息视频"];
						// this.webList = res.results["前端"];
						// this.expandList = res.results["拓展资源"];
						// this.recommendList = res.results["瞎推荐"];
